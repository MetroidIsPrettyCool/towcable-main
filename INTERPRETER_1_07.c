...
int AEND (int *tokens, int tokenAmount, int *cell, int *cellSizes, int dimensions, int *cellPointerCoors, int *pastCell)  {
  ...
  for (;;)  {
    ...
    printf(" \b");
    if (currMultiOp != 10)  {
      for (i2 = 0; i2 != ((currMultiOp == 308) ? dimensions : ((currMultiOp == 306) ? dimensions + 1 : ((currMultiOp == 307) ? (2 * dimensions) : 2))); i2++)  {
  ...
  for (;;)  {
	...
	*(cellPointerCoors + i) = floor((*(ruleLocs + i4) / i3) % i2);
	...
	if ((input < 0 || input == 100 || input == 102 || input == 1738 || (input >= 200 && input < 203)) && input != 7)  {
	  ...
	  tracker += ((input == 308) ? D : ((input == 306) ? D + 1 : ((input == 307) ? (2 * D) : 2))) - 1; // Figures out how many arguments each command takes
	}
	if (tracker != 0)  {

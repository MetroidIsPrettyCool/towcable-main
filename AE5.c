...
int AEND (int *tokens, int tokenAmount, int *cell, int *cellSizes, int dimensions, int *cellPointerCoors, int *pastCell)  {
  ...
  for (i = tokenAmount; (*(tokens + i) > 306 || *(tokens + i) < 200) && *(tokens + i) != 10; i--);
  for (;;)  { 
	...
	*(tokens + currLoc) = (*(cell + i3) + 1) * -1;
	...
	for (i = tokenAmount; (*(tokens + i) > 306 || *(tokens + i) < 300) && *(tokens + i) != 10; i--);
      }
      if (currMultiOp == 201)  {
	...
	*(tokens + currLoc) = (*(cell + i3) + 1) * -1;
	for (i = tokenAmount; (*(tokens + i) > 306 || *(tokens + i) < 300) && *(tokens + i) != 10; i--);
      ...
      if (currMultiOp == 300)  {
 	*(tokens + currLoc) = (rand() % (*(valS + 1))) + *valS;
 	*(tokens + currLoc) = (*(tokens + currLoc) + 1) * -1;
 	for (i = tokenAmount; (*(tokens + i) > 306 || *(tokens + i) < 300) && *(tokens + i) != 10; i--);
      }
      if (currMultiOp == 301)  {
 	*(tokens + currLoc) = *valS + *(valS + 1);
 	*(tokens + currLoc) = (*(tokens + currLoc) + 1) * -1;
 	for (i = tokenAmount; (*(tokens + i) > 306 || *(tokens + i) < 300) && *(tokens + i) != 10; i--);
      }
      if (currMultiOp == 302)  {
...
int main (void)  {
  ...
  // Variables
  int tokenAmount = 3; //Amount of tokens in the token array

...
________________________________________________________________________________________________________________________________________________________________________________________________________  
  Token values:
     ...
     Numerical Ops : RAND = 300 ADDN = 301 SUBN = 302 MULN = 303 DIV = 304 MODN = 305 PAST = 306 LOCN = 307 ATLO = 308
...
int AEND (int *tokens, int tokenAmount, int *cell, int *cellSizes, int dimensions, int *cellPointerCoors, int *pastCell)  {
  ...
  for (;;)  { // Main loop (is escaped by a return, I believe)
    ...
    if (currMultiOp != 10)  { // While we haven't reached the end of the token array
      for (i2 = 0; i2 != ((currMultiOp == 308) ? dimensions : ((currMultiOp == 306) ? (2 * dimensions) + 1 : ((currMultiOp == 307) ? (2 * dimensions) : 2))); i2++)  { // this loop fetches all the arguments for the currMultiOp and zeros their locations in the array
  ...
  for (;;)  {
	...
	*(cellPointerCoors + i) = floor((*(ruleLocs + i4) / i3) % i2);
	...
	if ((input < 0 || input == 100 || input == 102 || input == 1738 || (input >= 200 && input < 203)) && input != 7)  {
	  ...
	  tracker += ((input == 308) ? D : ((input == 306) ? (2 * D) + 1 : ((input == 307) ? (2 * D) : 2))) - 1; // Figures out how many arguments each command takes
	}
	if (tracker != 0)  {

...
________________________________________________________________________________________________________________________________________________________________________________________________________  
  Token values:
     ...
     Numerical Ops : RAND = 300 ADDN = 301 SUBN = 302 MULN = 303 DIV = 304 MODN = 305 PAST = 306 LOCN = 307 ATLO = 308 RVAL = 309 ROOT = 310
...
int AEND (int *tokens, int tokenAmount, int *cell, int *cellSizes, int dimensions, int *cellPointerCoors, int *pastCell, int *registers)  {
  ...
  for (;;)  { // Main loop (is escaped by a return, I believe)
    ...
    if (currMultiOp != 10)  { // While we haven't reached the end of the token array
      for (i2 = 0; i2 != ((currMultiOp == 306) ? (2 * dimensions) + 1 : ((currMultiOp == 307 || currMultiOp == 308) ? (2 * dimensions) : ((currMultiOp == 309) ? 1 : 2))); i2++)  { // this loop fetches all the arguments for the currMultiOp and zeros their locations in the array
	...
	*(tokens + currLoc) = (i3 + 1) * -1; // Return result
      }
      if (currMultiOp == 309)  { // RVAL
	*(tokens + currLoc) = (-1 * *(registers + *valS)) - 1;
      }
      if (currMultiOp == 310)  { // ROOT
 	*(tokens + currLoc) = (int)((pow(*valS, (1.0/(*(valS + 1)))) + 1) * -1);
      }
  ...
  for (;;)  {
	...
	*(cellPointerCoors + i) = floor((*(ruleLocs + i4) / i3) % i2);
	...
	currCommand = 612;
      ...
      else if (*(inptStr) == 'S' && *(inptStr + 1) == 'T' && *(inptStr + 2) == 'O' && *(inptStr + 3) == 'P')  {
	currCommand = 613;
      }
      else  {
	endwin();
	...
	if ((input < 0 || input == 100 || input == 102 || input == 1738 || (input >= 200 && input < 203)) && input != 7)  {
	  ...
	  tracker += ((input == 306) ? ((2 * D) + 1) : ((input == 307 || input == 308) ? (2 * D) : ((input == 309) ? 1 : 2))) - 1; // Figures out how many arguments each command takes
	}
	if (tracker != 0)  {
	  if (isdigit(*(inptStr + inStrPtr)))  { // Gets numbers
	    ...
	    else if (*(inptStr + inStrPtr) == 'R' && *(inptStr + inStrPtr + 1) == 'O' && *(inptStr + inStrPtr + 2) == 'O' && *(inptStr + inStrPtr + 3) == 'T')  {
	      input = 310;
	      inStrPtr += 4;
	    }
	...
	*(registers + *arguments) = 0; // Initiallize the register
	...
	if (*arguments > largestRegisterID || *(allocedRegisters + *arguments) == 0)  {
	  endwin();
	  printf("ERR 26: ATTEMPTING TO SET VALUE OF NON-EXISTANT REGISTER\n");
	  return -1;
	}
	scanw("%d%s", (registers + *arguments), NULL);
	if (*(registers + *arguments) < 0)  {
